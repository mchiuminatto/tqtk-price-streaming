...
- **THEN** every subscribed consumer whose filter matches the update's `provider`/`symbol`/
  `timeframe` receives it over its WebSocket connection

### Requirement: Subscription filtering
A consumer SHALL be able to subscribe with a filter on `provider`, `symbol`, and `timeframe`, and
SHALL receive only messages matching that filter.

#### Scenario: Consumer subscribes with a filter
- **WHEN** a consumer subscribes specifying `provider`, `symbol`, and `timeframe`
- **THEN** it receives only updates matching that `(provider, symbol, timeframe)` combination
...
`(provider, symbol, timeframe)` key, and SHALL drop further backlog beyond a bound rather than
growing the queue unboundedly or blocking the fast path.
...
The gateway SHALL expose a read endpoint that, given `provider`, `symbol`, `timeframe`, and
`count = N`, returns one ordered, gapless list: the `N-1` most recent closed bars followed by the
1 currently-forming bar.

#### Scenario: Consumer requests a snapshot
- **WHEN** a consumer requests a snapshot for `(provider, symbol, timeframe, count=N)`
- **THEN** it receives the `N-1` most recent closed bars followed by the 1 currently-forming bar,
...
on `(provider, symbol, timeframe, bar_start_ts)`, keeping the closed version.
...
- **THEN** the response is reconciled on `(provider, symbol, timeframe, bar_start_ts)` so it
