...
- [ ] 1.1 Create `tools/calibration_seed/` with the venue/file symbology table, the per-instrument
...
- [ ] 1.2 Move the per-symbol fit rationale from `docs/tick-distributions.md`,
...
- [ ] 1.3 Add a one-time equivalence test asserting, for all 17 symbols, that the tool's
      ...
      code.
- [ ] 1.4 Implement code-unit → stored-unit conversion in `Decimal` (spread `loc`/`scale` ÷
...
- [ ] 1.5 Implement the seeder: scan existing `calib:*`, `instrument:*` and `symbology` keys, then
      ...
      key.
- [ ] 1.6 Add `tools/calibration_seed/__main__.py` so `python -m tools.calibration_seed` seeds
...
- [ ] 2.1 Add `calibration_store.py` with `CalibrationStore` (injected `redis.asyncio.Redis`,
...
- [ ] 2.2 Make the load fail fast: empty or absent `calib:symbols`, a symbol missing from
...
- [ ] 2.3 Add the round-trip test in the tool's suite: seed into the fake, load through
...
- [ ] 2.4 Wire `__main__._run_service`: `Readiness("redis", "calibration")`, load calibration
...
- [ ] 2.5 Add any new `FeedConfig` field the store needs (e.g. a calibration Redis URL if it is
      not the bus) or record that none is needed; verify `FeedConfig` still rejects unknown
      `TQTK_` variables.

## 3. Deployment (`calibration-store` spec)

- [ ] 3.1 Add `tools/calibration_seed/Dockerfile` (built from the repo root, installing only
...
- [ ] 3.2 Make `feed-adapter-synthetic` depend on `calibration-seeder` with
...
- [ ] 4.1 Remove the three distribution tables, their accessors, `_lookup` and
...
- [ ] 4.2 Reduce `calibration.py` to `SymbolCalibration`, delete `symbols.py`, drop
...
- [ ] 4.3 Drop `pyarrow>=17.0` from the service's `pyproject.toml` and refresh `uv.lock`; verify
      `grep -rn "pyarrow" services/feed-adapter-synthetic/` returns nothing.
- [ ] 4.4 Delete `docs/tick-distributions.md`, `docs/spread-distributions.md`,
...
- [ ] 5.1 Run `openspec validate move-calibration-to-redis --strict`; verify it reports the change
      valid.
- [ ] 5.2 Run `pytest` at the repo root and `ruff check .`; verify both pass.
- [ ] 5.3 Run `docker compose up` in `deploy/`; verify the seeder exits `0`, the adapter's `/ready`
      ...
      no `data/` mount.
- [ ] 5.4 Stop the stack, flush Redis, and start only Redis and the adapter; verify the adapter
      publishes nothing and exits with an error stating the store is not seeded.
